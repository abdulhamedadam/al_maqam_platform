                                                    ...
                                                    @if(isset($data['appointment']))
                                                        <div class="notification-details mt-2 p-2 bg-light rounded">
                                                            <small class="text-muted">
                                                                ...
                                                                @if($notificationType === 'AppointmentCancelledNotification' && isset($data['cancelled_by']))
                                                                    <br><strong>{{ trans('notifications.cancelled_by') }}:</strong> {{ $data['cancelled_by'] }}
                                                                @endif
                                                            </small>
                                                        </div>
                                                    @endif
...
<script>
    ...
    function markAsRead(notificationId) {
        $.ajax({
            url: '{{ route("user.student.notifications.mark_read", "") }}/' + notificationId,
            method: 'POST',
            headers: {
    ...
    function markAllAsRead() {
        ...
        $.ajax({
            url: '{{ route("user.student.notifications.mark_all_read") }}',
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
