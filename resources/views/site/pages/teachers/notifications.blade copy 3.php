                            ...
                            <div class="tab-pane fade active show" id="notifications" role="tabpanel"
                                ...
                                <div class="container notifications-container">
                                    ...
                                    <div class="filter-tabs">
                                        ...
                                        <button class="filter-tab" data-filter="reminders">{{ trans('notifications.lecture_reminders') }}</button>
                                        <button class="filter-tab" data-filter="start_lecture">{{ trans('notifications.start_lecture') }}</button>
                                        <button class="filter-tab" data-filter="end_lecture">{{ trans('notifications.end_lecture_reminders') }}</button>
                                        <button class="filter-tab" data-filter="missed">{{ trans('notifications.missed_lectures') }}</button>
                                        <button class="filter-tab" data-filter="auto_ended">{{ trans('notifications.auto_ended_lectures') }}</button>
                                        <button class="filter-tab" data-filter="cancelled">{{ trans('notifications.cancelled_lectures') }}</button>
                                        <button class="filter-tab" data-filter="tomorrow">{{ trans('notifications.tomorrow_lectures') }}</button>
                                    </div>
                                                        ...
                                                        <small class="text-muted">
                                                            @switch($notificationType)
                                                                @case('AppointmentCancelledNotification')
                                                                    ...
                                                                    <strong>{{ trans('notifications.cancelled_time') }}:</strong>
                                                                    {{ format_time_arabic($data['start_time'] ?? '') }} - {{ format_time_arabic($data['end_time'] ?? '') }}<br>
                                                                    <strong>{{ trans('notifications.day') }}:</strong> {{ $data['day'] ?? 'N/A' }}<br>
                                                                    <strong>{{ trans('notifications.cancelled_by') }}:</strong> {{ $data['cancelled_by'] ?? 'Student' }}<br>
                                                                    @if(isset($data['cancellation_reason']))
                                                                        <strong>{{ trans('notifications.reason') }}:</strong> {{ $data['cancellation_reason'] }}<br>
                                                                    @endif
                                                                    <span class="text-danger">
                                                                        <i class="fa fa-times-circle"></i> {{ trans('notifications.lecture_cancelled') }}
                                                                    </span>
                                                                    @break
                                                                ...
                                                                @case('LectureReminderNotification')
                                                                    ...
                                                                    {{ format_time_arabic($data['start_time']) }} - {{ format_time_arabic($data['end_time']) }}<br>
                                                                    <strong>{{ trans('notifications.day') }}:</strong> {{ $data['day'] ?? 'N/A' }}
                                                                    @break
                                                                ...
                                                                @case('StartLectureNotification')
                                                                    ...
                                                                    @if(isset($data['zoom_link']))
                                                                        ...
                                                                        <a href="{{ $data['zoom_link'] }}" target="_blank" class="btn btn-sm btn-primary">
                                                                            <i class="fa fa-video"></i> {{ trans('notifications.start_lecture') }}
                                                                        </a>
                                                                    @endif
                                                                    @break
                                                                ...
                                                                @case('EndLectureReminderNotification')
                                                                    <strong>{{ trans('notifications.student') }}:</strong> {{ $data['student_name'] ?? 'N/A' }}<br>
                                                                    <strong>{{ trans('notifications.course') }}:</strong> {{ $data['course_name'] ?? 'N/A' }}<br>
                                                                    <strong>{{ trans('notifications.lecture_started') }}:</strong> {{ format_time_arabic($data['start_time']) }}<br>
                                                                    <strong>{{ trans('notifications.should_end') }}:</strong> {{ format_time_arabic($data['end_time']) }}<br>
                                                                    <strong>{{ trans('notifications.overtime') }}:</strong>
                                                                    <span class="text-warning">{{ $data['overtime_minutes'] ?? '15' }} {{ trans('notifications.minutes') }}</span>
                                                                    @break

                                                                @case('MissedLectureNotification')
                                                                    <strong>{{ trans('notifications.student') }}:</strong> {{ $data['student_name'] ?? 'N/A' }}<br>
                                                                    <strong>{{ trans('notifications.course') }}:</strong> {{ $data['course_name'] ?? 'N/A' }}<br>
                                                                    <strong>{{ trans('notifications.missed_time') }}:</strong>
                                                                    {{ format_time_arabic($data['start_time']) }} - {{ format_time_arabic($data['end_time']) }}<br>
                                                                    <strong>{{ trans('notifications.day') }}:</strong> {{ $data['day'] ?? 'N/A' }}<br>
                                                                    <span class="text-danger">
                                                                        <i class="fa fa-exclamation-triangle"></i> {{ trans('notifications.lecture_not_started') }}
                                                                    </span>
                                                                    @break
                                                                ...
                                                                @case('AutoEndedLectureNotification')
                                                                    ...
                                                                    <span class="text-warning">1 {{ trans('notifications.hour_overtime') }}</span><br>
                                                                    <span class="text-info">
                                                                        <i class="fa fa-robot"></i> {{ trans('notifications.system_auto_ended') }}
                                                                    </span>
                                                                    @break
                                                                ...
                                                                @case('TomorrowLectureNotification')
                                                                    <strong>{{ trans('notifications.student') }}:</strong> {{ $data['student_name'] ?? 'N/A' }}<br>
                                                                    <strong>{{ trans('notifications.course') }}:</strong> {{ $data['course_name'] ?? 'N/A' }}<br>
                                                                    <strong>{{ trans('notifications.tomorrow_time') }}:</strong>
                                                                    {{ format_time_arabic($data['start_time']) }} - {{ format_time_arabic($data['end_time']) }}<br>
                                                                    <strong>{{ trans('notifications.day') }}:</strong> {{ $data['day'] ?? 'N/A' }}<br>
                                                                    <span class="text-info">
                                                                        <i class="fa fa-calendar-alt"></i> {{ trans('notifications.prepare_for_tomorrow') }}
                                                                    </span>
                                                                    @break
        ...
        function filterNotifications(filter) {
            ...
            switch(filter) {
                ...
                case 'unread':
                    ...
                    break;
                case 'reminders':
                    $('.notification-item[data-type*="lecturereminder"]').show();
                    hasVisibleItems = $('.notification-item[data-type*="lecturereminder"]').length > 0;
                    break;
                case 'start_lecture':
                    ...
                    break;
                case 'end_lecture':
                    $('.notification-item[data-type*="endlecturereminder"]').show();
                    hasVisibleItems = $('.notification-item[data-type*="endlecturereminder"]').length > 0;
                    break;
                case 'missed':
                    $('.notification-item[data-type*="missedlecture"]').show();
                    hasVisibleItems = $('.notification-item[data-type*="missedlecture"]').length > 0;
                    break;
                case 'auto_ended':
                    $('.notification-item[data-type*="autoendedlecture"]').show();
                    hasVisibleItems = $('.notification-item[data-type*="autoendedlecture"]').length > 0;
                    break;
                case 'cancelled':
                    ...
                    break;
                case 'tomorrow':
                    $('.notification-item[data-type*="tomorrowlecture"]').show();
                    hasVisibleItems = $('.notification-item[data-type*="tomorrowlecture"]').length > 0;
                    break;
            }
