                                    ...
                                    <div class="notifications-list">
                                        @forelse($notifications as $notification)
                                            @php
                                                ...
                                                $notificationType = $data['type'] ?? class_basename($notification->type);

                                                $iconClass = get_notification_icon($notificationType);
                                                $colorClass = 'type-' . get_notification_color($notificationType);
                                                $typeLabel = trans('notifications.' . $notificationType);
                                            @endphp
                                                    ...
                                                    <div class="notification-details mt-2 p-2 bg-light rounded">
                                                        <small class="text-muted">
                                                            @switch($notificationType)
                                                                ...
                                                                @case('start_lecture')
                                                                    <strong>{{ trans('notifications.student') }}:</strong> {{ $data['student_name'] ?? 'Student' }}<br>
                                                                ...
                                                                @case('auto_ended_lecture')
                                                                    <strong>{{ trans('notifications.student') }}:</strong> {{ $data['student_name'] ?? 'Student' }}<br>
                                                                    <strong>{{ trans('notifications.teacher') }}:</strong> {{ $data['teacher_name'] ?? 'Teacher' }}<br>
                                                                    <strong>{{ trans('notifications.lecture_duration') }}:</strong>
                                                                    {{ format_time_arabic($data['start_time']) }} - {{ format_time_arabic($data['end_time']) }}<br>
                                                                    <strong>{{ trans('notifications.day') }}:</strong> {{ $data['day'] ?? 'N/A' }}<br>
                                                                    <strong>{{ trans('notifications.auto_ended_after') }}:</strong>
                                                                    <span class="text-warning">{{ $data['overtime_minutes'] ?? '60' }} {{ trans('notifications.minutes_overtime') }}</span><br>
                                                                ...
                                                                {{-- @case('lecture_started')
                                                                    ...
                                                                    <span class="text-success">
                                                                        <i class="fa fa-play-circle"></i> {{ trans('notifications.lecture_has_started') }}
                                                                    </span>
                                                                    @break --}}
                                                                ...
                                                                @case('end_lecture')
                                                                    ...
                                                                    <strong>{{ trans('notifications.day') }}:</strong> {{ $data['day'] ?? 'N/A' }}<br>
                                                                    <span class="text-info">
                                                                        <i class="fa fa-stop-circle"></i> {{ trans('notifications.lecture_ended') }}
                                                                    </span>
                                                                    @break

                                                                @default
                                                                    @if(isset($data['appointment_id']))
                                                                        <strong>{{ trans('notifications.appointment_id') }}:</strong> {{ $data['appointment_id'] }}<br>
                                                                    @endif
    ...
    $(document).ready(function() {
        ...
        function filterNotifications(filter) {
            ...
            switch(filter) {
                ...
                //     break;
                case 'end_lecture':
                    $('.notification-item[data-type="end_lecture"]').show();
                    hasVisibleItems = $('.notification-item[data-type="end_lecture"]').length > 0;
                    break;
            }
